...
def construct_fields( model, registry, only_fields, exclude_fields, convert_choices_to_enum ):
    ...
    for name, field in _model_fields:
        is_not_in_only = ( only_fields is not None and only_fields != ALL_FIELDS and name not in only_fields )
        # is_already_created = name in options.fields
        is_excluded = ( exclude_fields is not None and name in exclude_fields )  # or is_already_created
        ...
        converted = convert_django_field_with_choices( field, FieldPurposeConvert.OUTPUT.value, registry, convert_choices_to_enum=_convert_choices_to_enum )

        if isinstance( field, (ManyToManyField, ManyToManyRel, ManyToOneRel) ) and not isinstance(field, OneToOneRel): # TODO:
...
class DjangoObjectTypeOptions(ObjectTypeOptions):
    ...
    registry = None  # type: RegistryGlobal
...
class DjangoObjectType(ObjectType):
    ...
    def __init_subclass_with_meta__(
        ...
        **options
        ...
        assert isinstance(registry, RegistryGlobal), (
            "The attribute registry in {} needs to be an instance of "
            'RegistryGlobal, received "{}".'
        ...
        django_fields = yank_fields_from_attrs( construct_fields(model, registry, fields, exclude, convert_choices_to_enum), _as=graphene.Field, )
