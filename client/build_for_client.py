from ..utils.utils import transform_string
import os
import json
...
from graphql.utilities.print_schema import print_schema, print_introspection_schema
...
def build_file_general_types():
    with open(f'{PATH_CLIENT}/general_types.js','w+') as file:
        file.write("export const PaginatedType = `\n  total\n  page\n  pages\n  hasNext\n  hasPrev\n  indexStartObj\n  indexEndObj\n`;\n")
        file.write("export const ErrorsType = `\n  objectPosition\n  errors {\n    field\n    messages\n }\n`;\n")
    ...
    if schema is not None:
        ...
        for app_name in apps_name:
            ...
            for model in app.get_models():
                ...
                model_name_plural = transform_string(model._meta.verbose_name_plural, 'PascalCase')

                globals()["global_cruddals_strings"][app_name].update({ model_name: {} })

                query_read = build_read(app_name, model_name, queries)
                query_list = build_list(app_name, model_name, model_name_plural, queries)
                query_search = build_search(app_name, model_name, model_name_plural, queries)
                ...
                if mutations:
                    mutation_create = build_create(app_name, model_name, model_name_plural, mutations)
                    mutation_update = build_update(app_name, model_name, model_name_plural, mutations)
                    mutation_delete = build_delete(app_name, model_name, model_name_plural, mutations)
                    mutation_deactivate = build_deactivate(app_name, model_name, model_name_plural, mutations)
                    mutation_activate = build_activate(app_name, model_name, model_name_plural, mutations)
                
                

                text_queries += f"\n//region ============= {model_name.upper()}{query_read}{query_list}{query_search}\n//endregion\n"
                if mutation_create or mutation_update or mutation_delete or mutation_deactivate or mutation_activate:
...
def build_files_for_client_schema(schema, name):
    ...
    def get_first_scalar_field(field, path=None):
        ...
        if isinstance(field.type, graphene.List) or isinstance(field.type, graphene.NonNull):
    ...
    if schema is not None:
        ...
        for name_query, definition_graphql_field in queries.items():
            args_str, vars_args_str = return_args_and_variables(definition_graphql_field)
